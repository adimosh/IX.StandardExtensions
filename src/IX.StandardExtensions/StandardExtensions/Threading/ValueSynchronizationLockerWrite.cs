...
public readonly struct ValueSynchronizationLockerWrite : IDisposable
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ValueSynchronizationLockerWrite"/> struct.
    /// </summary>
    /// <param name="locker">The locker to use.</param>
    public ValueSynchronizationLockerWrite(IReaderWriterLock? locker)
        : this(
            locker,
    ...
    public ValueSynchronizationLockerWrite(
    ...
    public void Dispose()
    {
        if (_locker is not null && _locker.IsWriteLockHeld)
        {
